...
#outputs<-outputs[1:100000, ]
# nrow(outputs)
# head(outputs)
saveRDS(outputs, "./Documents/R_learnings/R_group_troubleshoot/outputs_small.rds")
...
distance.fun<-function(i){
  ...
  out1<-data.frame(plot=integer(), Sigma_bin=double(), mean_resid_km=double())   # set an empty dataframe to collect 
  plot<-focals1[i]                                                               # outputs
  output_data<-filter(out_chnk1, focal_plot==plot)
  dist_table<-filter(dist_per_plot, focal_plot==plot)
